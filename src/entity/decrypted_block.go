...
func (d *DecryptedBlock) ToBytes() []byte {
	b := make([]byte, 16)
	utils.WriteIntLong(b, 0, int64((d.clientId<<20)|d.expirationDate))
	utils.WriteIntLong(b, 4, int64(d.randomSize))
	// 保留的8字节默认已初始化为0
	return b
}
...
func Decrypt(key string, encryptedBytes []byte) (*DecryptedBlock, error) {
	...
	decryptedBlock.SetClientId(int(utils.GetIntLong(decryptedBytes, 0) >> 20))
	decryptedBlock.SetExpirationDate(int(utils.GetIntLong(decryptedBytes, 0) & 0xFFFFF))
	decryptedBlock.SetRandomSize(int(utils.GetIntLong(decryptedBytes, 4)))
