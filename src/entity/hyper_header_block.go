...
import "cz-ip-service/src/utils"
...
func (h *HyperHeaderBlock) ToBytes() []byte {
	bytes := make([]byte, 12)
	utils.WriteIntLong(bytes, 0, h.Version)
	utils.WriteIntLong(bytes, 4, h.ClientId)
	utils.WriteIntLong(bytes, 8, int64(h.EncryptedBlockSize))
	return bytes
}
...
func FromBytes(bytes []byte) *HyperHeaderBlock {
	version := utils.GetIntLong(bytes, 0)
	clientId := utils.GetIntLong(bytes, 4)
	encryptedBlockSize := utils.GetIntLong(bytes, 8)
