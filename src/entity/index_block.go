...
func NewIndexBlock(startIp, endIp []byte, dataPtr, dataLen int, dbType constant.DbType) *IndexBlock {
	return &IndexBlock{
		startIp: startIp,
		endIp:   endIp,
		dataPtr: dataPtr,
...
func GetIndexBlockLength(dbType constant.DbType) int {
...
func (ib *IndexBlock) GetBytes() []byte {
	...
	// 写入数据指针和长度
	utils.WriteIntLong(b, ipBytesLength*2, int64(ib.dataPtr))
	utils.Write(b, ipBytesLength*2+4, int64(ib.dataLen), 1)
	return b
}
