...
import (
	...
	"cz-ip-service/src/entity"
	"cz-ip-service/src/hyperHeaderDecoder"
...
func NewDbSearcher(dbFile string, queryType constant.QueryType, key string) (*DbSearcher, error) {
	ds := &DbSearcher{
		queryType: queryType,
	}
	...
	superBytes := make([]byte, constant.BlockSizePtr)
	...
	if queryType == constant.MEMORY {
		err = ds.initializeForMemorySearch()
	} else if queryType == constant.BTREE {
...
func (ds *DbSearcher) loadGeoSetting(raf *RandomAccessFile, key string) {
	...
	_, err := raf.Seek1(constant.EndIndexPtr)
	if err != nil {
		return
	}
	data := make([]byte, 4)
	...
	endIndexPtr := utils.GetIntLong(data, 0)
	...
	ds.columnSelection = utils.GetIntLong(data, 0)
	...
	geoMapSize := int(utils.GetIntLong(data, 0))
	...
	ds.geoMapData = decryptor.decrypt(ds.geoMapData)
	return
}
...
func (ds *DbSearcher) initMemoryOrBinaryModeParam(bytes []byte, fileSize int64) error {
	ds.totalHeaderBlockSize = utils.GetIntLong(bytes, constant.HeaderBlockPtr)
	fileSizeInFile := utils.GetIntLong(bytes, constant.FileSizePtr)
	if fileSizeInFile != fileSize {
		return fmt.Errorf("db file size error, excepted [%d], real [%d]", fileSizeInFile, fileSize)
	}
	ds.firstIndexPtr = utils.GetIntLong(bytes, constant.FirstIndexPtr)
	lastIndexPtr := utils.GetIntLong(bytes, constant.EndIndexPtr)
	...
	copy(b, bytes[constant.BlockSizePtr:])
...
func (ds *DbSearcher) initBtreeModeParam(raf *RandomAccessFile) error {
	...
	ds.totalHeaderBlockSize = utils.GetIntLong(superBytes, constant.HeaderBlockPtr)

	fileSizeInFile := utils.GetIntLong(superBytes, constant.FileSizePtr)
...
func (ds *DbSearcher) initHeaderBlock(headerBytes []byte) error {
	...
	for i := 0; i < len(headerBytes); i += indexLength {
		dataPtr = utils.GetIntLong(headerBytes, i+16)
		if dataPtr == 0 {
			break
		}
...
func (ds *DbSearcher) memorySearch(ip []byte) *entity.DataBlock {
	...
	sptrNeptr := ds.searchInHeader(ip)
	sptr, eptr := sptrNeptr[0], sptrNeptr[1]

	if sptr == 0 {
		return nil
	}

	l, h := 0, (eptr-sptr)/blockLen
	...
	for l <= h {
		...
		if cmpStart >= 0 && cmpEnd <= 0 {
			dataPtr = int(utils.GetIntLong(ds.dbBinStr, p+ds.ipBytesLength*2))
			dataLen = utils.GetInt1(ds.dbBinStr, p+ds.ipBytesLength*2+4)
			break
		} else if cmpStart < 0 {
			h = m - 1
		} else {
...
func (ds *DbSearcher) searchInHeader(ip []byte) [2]int {
	...
	if l > h {
		if l < ds.headerLength {
			sptr = ds.headerPtr[l-1]
			eptr = ds.headerPtr[l]
		} else if h >= 0 && h+1 < ds.headerLength {
			sptr = ds.headerPtr[h]
			eptr = ds.headerPtr[h+1]
		} else {
			sptr = ds.headerPtr[ds.headerLength-1]
			eptr = sptr + entity.GetIndexBlockLength(ds.dbType)
		}
	}

	return [2]int{sptr, eptr}
}
...
func (ds *DbSearcher) bTreeSearch(ip []byte) (*entity.DataBlock, error) {
	...
	for l <= h {
		...
		if cmpStart >= 0 && cmpEnd <= 0 {
			dataPtr = int(utils.GetIntLong(iBuffer, p+ds.ipBytesLength*2))
			dataLen = utils.GetInt1(iBuffer, p+ds.ipBytesLength*2+4)
			break
		} else if cmpStart < 0 {
			h = m - 1
		} else {
...
func (ds *DbSearcher) getByIndexPtr(ptr int64) (*entity.DataBlock, error) {
	...
	extra := utils.GetIntLong(buffer, 32)
...
func (ds *DbSearcher) getDbType() constant.DbType {
	return ds.dbType
}
func (ds *DbSearcher) getQueryType() constant.QueryType {
	return ds.queryType
}
